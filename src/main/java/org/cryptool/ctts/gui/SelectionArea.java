...
public class SelectionArea extends HBox {
        ...
        importIconButton.setOnMousePressed(event -> {
            if (selectedColor != null) {
                ...
                if (Icons.readIcon(selectedColor, selectedFile.getAbsolutePath(), false)) {
                    ...
                    CTTSApplication.colorParametersChanged(selectedColor, false, true, false);
                }
            }
        });
    ...
    public void updateLocked(boolean newState) {
        ...
        String p = CTTSApplication.key.fromTranscriptionOrDefault(c, "");
        if (p == null) {
            p = "";
        }
        ...
        if (newState) {
            if (!Key.lockedC(c) && !CTTSApplication.key.lockedP(c)) {
                if (c.isEmpty()) {
                    CTTSApplication.colors.put(selectedColor.toString(), "_");
            ...
            if (Key.lockedC(c) && CTTSApplication.key.lockedP(c)) {
                ...
                CTTSApplication.key.put(c, p);
            } else if (CTTSApplication.key.lockedP(c)) {
                while ((p.length() >= 1 && p.charAt(0) >= 'A' && p.charAt(0) <= 'Z') || p.startsWith("_")) {
                    if (p.charAt(0) >= 'A' && p.charAt(0) <= 'Z') {
                        p = p.substring(0, 1).toLowerCase() + p.substring(1);
                    }
        ...
        CTTSApplication.colorParametersChanged(selectedColor, transcriptionChanged, false, true /* to be on the safe side */);
    }
    ...
    public void selectColor(Color color, boolean force) {
        ...
        if (colorChanged || force) {
            decryptionTextField.setVisible(CTTSApplication.key.isKeyAvailable());
            lockUnlock.setVisible(CTTSApplication.key.isKeyAvailable());
            ...
            transcriptionTextField.setText(CTTSApplication.colors.get(color.toString()));
    ...
    private void updateDecryptionFields(Color color, boolean setSelected) {
        ...
        if (CTTSApplication.key.fromTranscriptionAvailable(c)) {
            plaintext = CTTSApplication.key.fromTranscription(c);
        }
        decryptionTextField.setText(plaintext);

        if (setSelected) {
            lockUnlock.setSelected(CTTSApplication.key.lockedP(c) || Key.lockedC(c));
        }
    }
